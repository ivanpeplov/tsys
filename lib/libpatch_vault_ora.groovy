def set_global_env() {
    ...
    if (params.xmlConfigBase64) { xmlConfig = new String(xmlConfigBase64.decodeBase64()) }
        ...
        println("Params XML file & REGION at the same time")
        currentBuild.result = 'FAILURE' }
...
def db_ops(String operation) {
    node ("${REGION}") {
        dir('prime4db'){
            //prime_app_usr SCHEMA1:PRM_APP0000 / SCHEMA2:PRM_APP0000_2
            prime_app_usr='PRM_APP0000'
            //HCV vault credentials hosted at .161(nexus)
            def ora_creds = [
            [path: 'secrets/creds/ora/${REGION}/ADM', secretValues: [
                [envVar: 'deployment_adm_pwd', vaultKey: 'password']]],
            [path: 'secrets/creds/ora/${REGION}/DBS', secretValues: [
                [envVar: 'data_schema_pwd', vaultKey: 'password']]],
            [path: 'secrets/creds/ora/${REGION}/EBR', secretValues: [
                [envVar: 'ebr_pwd', vaultKey: 'password']]],
            [path: 'secrets/creds/ora/${REGION}/PRM', secretValues: [
                [envVar: 'prime_app_pwd', vaultKey: 'password']]],
            [path: 'secrets/creds/ora/${REGION}/STRM', secretValues: [
                [envVar: 'tctstrm_pwd', vaultKey: 'password']]],
            [path: 'secrets/creds/ora/${REGION}/TCT', secretValues: [
                [envVar: 'code_schema_pwd', vaultKey: 'password'],
                [envVar: 'code_schema2_pwd', vaultKey: 'password2']]]]
            switch(operation) {
                case "streams_stop":
                ...
                case "install_rdsadmin":
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_rdsadmin.sh ${DB_PRIME_LIQUIBASE_URL} DEPLOYMENTADM "+'${deployment_adm_pwd}'+" RDSADMIN ${RDSADMIN_DIRECTORIES_PATH} /oracle/app/scripts DEPLOY_TBS TEMP"
                    sh  "./install_rdsadmin.sh ${DB_ONLINE_LIQUIBASE_URL} DEPLOYMENTADM "+'${deployment_adm_pwd}'+" RDSADMIN ${RDSADMIN_DIRECTORIES_PATH} /oracle/app/scripts DEPLOY_TBS TEMP" }
                break
                case "install_admin":
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_admin.sh ${SYSTEM_NAME_PRIME} ${DB_PRIME_LIQUIBASE_URL} DEPLOYMENTADM "+'${deployment_adm_pwd}'+" ${RDSADMIN_DIRECTORIES_PATH}"
                    sh  "./install_admin.sh ${SYSTEM_NAME_ONLINE} ${DB_ONLINE_LIQUIBASE_URL} DEPLOYMENTADM "+'${deployment_adm_pwd}'+" ${RDSADMIN_DIRECTORIES_PATH}" }
                break
                //dblinks section using vault secrets. Need to scale for all db_ops actions
                //regular: verifying checksum in liquibase only
                case "db_links":
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_db_links.sh ${SYSTEM_NAME_PRIME} ${DB_PRIME_LIQUIBASE_URL} DEPLOYMENTADM "+'${deployment_adm_pwd}'+" ${DB_HOST_ONLINE} ${DB_PORT_ONLINE} ${DB_SERVICE_ONLINE} ${DB_SERVICE_NAME_ONLINE}"+' ${code_schema_pwd} ${code_schema2_pwd} ${data_schema_pwd} ${tctstrm_pwd}'
                    sh  "./install_db_links.sh ${SYSTEM_NAME_ONLINE} ${DB_ONLINE_LIQUIBASE_URL} DEPLOYMENTADM "+'${deployment_adm_pwd}'+" ${DB_HOST_PRIME} ${DB_PORT_PRIME} ${DB_SERVICE_PRIME} ${DB_SERVICE_NAME_PRIME}"+' ${code_schema_pwd} ${code_schema2_pwd} ${data_schema_pwd} ${tctstrm_pwd}' }
                break
                case "install_ebr_prime":
                     wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_ebr_schema.sh ${SYSTEM_NAME_PRIME} ${DB_PRIME_LIQUIBASE_URL} "+'${ebr_pwd}' }
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_data_schema.sh ${SYSTEM_NAME_PRIME} ${DB_PRIME_LIQUIBASE_URL} "+'${data_schema_pwd}' }
                break
                case "install_ebr_online":
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_ebr_schema.sh ${SYSTEM_NAME_ONLINE} ${DB_ONLINE_LIQUIBASE_URL} "+'${ebr_pwd}' }
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_data_schema.sh ${SYSTEM_NAME_ONLINE} ${DB_ONLINE_LIQUIBASE_URL} "+'${data_schema_pwd}' }
                break
                case "install_code_schema": //schema1 schema2
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_code_schema.sh ${SYSTEM_NAME_PRIME} ${DB_PRIME_LIQUIBASE_URL} TCTCD${SCHEMA} "+'${code_schema_pwd}'
                    sh  "./install_code_schema.sh ${SYSTEM_NAME_ONLINE} ${DB_ONLINE_LIQUIBASE_URL} TCTCD${SCHEMA} "+'${code_schema_pwd}' }
                break
                case "install_tctstrm_schema":
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install__tctstrm_schema.sh ${SYSTEM_NAME_PRIME} ${DB_PRIME_LIQUIBASE_URL} "+'${tctstrm_pwd}'
                    sh  "./install__tctstrm_schema.sh ${SYSTEM_NAME_ONLINE} ${DB_ONLINE_LIQUIBASE_URL} "+'${tctstrm_pwd}' }
                break
                case "install_institutions_admin":
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_institutions_admin.sh ${SYSTEM_NAME_PRIME} ${DB_PRIME_LIQUIBASE_URL} DEPLOYMENTADM "+'${deployment_adm_pwd}'+" ${INSTITUTION_LIST}"
                    sh  "./install_institutions_admin.sh ${SYSTEM_NAME_ONLINE} ${DB_ONLINE_LIQUIBASE_URL} DEPLOYMENTADM "+'${deployment_adm_pwd}'+" ${INSTITUTION_LIST}" }
                break
                case "install_post_code_schema": //schema1 schema2
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_post_code_schema.sh ${SYSTEM_NAME_PRIME} ${DB_PRIME_LIQUIBASE_URL} TCTCD${SCHEMA} "+'${code_schema_pwd}'
                    sh  "./install_post_code_schema.sh ${SYSTEM_NAME_ONLINE} ${DB_ONLINE_LIQUIBASE_URL} TCTCD${SCHEMA} "+'${code_schema_pwd}' }
                break
                case "install_post_tctstrm_schema":
                    wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                    sh  "./install_post_tctstrm_schema.sh ${SYSTEM_NAME_PRIME} ${DB_PRIME_LIQUIBASE_URL} "+'${tctstrm_pwd}' }
                break
                default:
...
def db_config() {
    node ("${REGION}") {
        dir('prime-configs') {
            //Masked PWD Credentials for Oracle DB from HashiCorp Vault
            def ora_creds = [
            [path: 'secrets/creds/ora/${REGION}/TCT', secretValues: [
            [envVar: 'code_schema_pwd', vaultKey: 'password']]]]
                wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) { //schema1 schema2
                sh  "./install.sh ${SYSTEM_NAME_PRIME} ${DB_PRIME_LIQUIBASE_URL} TCTDBS TCTCD${SCHEMA} "+'${code_schema_pwd}'+" ${CONFIG_INSTITUTION_LIST}" }
            if ( SCHEMA == "1" ) { 
                wrap([$class: 'VaultBuildWrapper', vaultSecrets: ora_creds]) {
                sh  "./install.sh ${SYSTEM_NAME_ONLINE} ${DB_ONLINE_LIQUIBASE_URL} TCTDBS TCTCD${SCHEMA} "+'${code_schema_pwd}'+" ${CONFIG_INSTITUTION_LIST}" }
            }
        }
    }
}
def state_onl_ncr(String app, String operation) {
...
def prime_web(String app) {
    for (server in tools.getServersByRole(schema: "${SCHEMA}", region: "${REGION}", role: app).split(' ')) {
        if (tools.getJenkinsSlave(name: "${server}")) { //check if there are hosts in schema 2. if NOT - skip stage
            ...
            //reusable code in libtools.groovy
...
//-----------------------------------------------------------------
//          __ ACSweb1___ ___ACSapp1___Onl_DB1_Sch1_______
//         |             V                                |
//         |             |                           Hsmi--Onl_App1
//      F5-|            NLB2                              |
//         |             |                           Hsmi--Onl_App2           
//         |___ACSweb2___A___ACSapp2___Onl_DB2_Sch2_______|
//                                     
...
Deployment status: ${currentBuild.result}
 """ ]
    if (currentBuild.result == 'SUCCESS'){
        ...
        else    { sh "echo 'Hi! Its only SCHEMA${env.SCHEMA} & ${env.REGION} region!'" }
    }
}
